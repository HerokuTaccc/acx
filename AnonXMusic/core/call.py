...
class Call(PyTgCalls):
    ...
    async def skip_stream(
        ...
        if video:
            ...
            stream = MediaStream(link, audio_parameters=AudioQuality.STUDIO, video_flags=MediaStream.Flags.IGNORE)
    ...
    async def stream_call(self, link):
        assistant: PyTgCalls= await group_assistant(self, config.LOGGER_ID)
    ...
    async def join_call(
        ...
        if video:
            ...
            stream = (
                ...
                if video
                else MediaStream(link, audio_parameters=AudioQuality.STUDIO, video_flags=MediaStream.Flags.IGNORE)
            )
        try:
            assistant: PyTgCalls
            await assistant.play(
        ...
        try:
            ...
            if "live_" in queued:
                ...
                stream = (
                    ...
                    else MediaStream(videoid, audio_parameters=AudioQuality.STUDIO, video_flags=MediaStream.Flags.IGNORE)
                )
                try:
                    await client.change_stream(chat_id, stream)
                except:
    ...
    async def decorators(self):
        ...
        @self.one.on_update(filters.stream_end(StreamEnded.Type.AUDIO))
        @self.two.on_update(filters.stream_end(StreamEnded.Type.AUDIO))
        @self.three.on_update(filters.stream_end(StreamEnded.Type.AUDIO))
        @self.four.on_update(filters.stream_end(StreamEnded.Type.AUDIO))
        @self.five.on_update(filters.stream_end(StreamEnded.Type.AUDIO))
